...
İlk altı production-candidate ders `sql.relational-thinking.001`, `sql.select-null-filtering.001`, `sql.aggregation-grain.001`, `sql.join-cardinality.001`, `sql.cte-subquery-sets.001` ve `sql.window-semantics.001` üzerinden uçtan uca çalışır. Ders; mental model, worked example, guided practice, independent production, debugging, transfer ve retention katmanlarını ayrı bölümler halinde gösterir.
