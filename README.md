...
İlk sekiz production-candidate ders `sql.relational-thinking.001`, `sql.select-null-filtering.001`, `sql.aggregation-grain.001`, `sql.join-cardinality.001`, `sql.cte-subquery-sets.001`, `sql.window-semantics.001`, `sql.analytical-patterns.001` ve `sql.model-quality.001` üzerinden uçtan uca çalışır. Ders; mental model, worked example, guided practice, independent production, debugging, transfer ve retention katmanlarını ayrı bölümler halinde gösterir.
...
Model-quality production dersi PK/UNIQUE/FK/CHECK constraint rollerini executable profiling ve reconciliation invariant'larıyla birleştirir. Semantic lab duplicate business keys, orphan foreign keys, invalid domain değerleri ve impossible numeric değerleri tek violation matrix'te visible + edge fixture üzerinde doğrular.
