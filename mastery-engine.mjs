export const DEFAULT_WEIGHTS=Object.freeze({knowledge:0.15,interpretation:0.20,production:0.35,transfer:0.30});
export const MASTERY_PROFILES=Object.freeze({
L1:{weightedMin:75,floors:{knowledge:85,interpretation:80,production:75,transfer:65}},
L2:{weightedMin:80,floors:{knowledge:85,interpretation:85,production:80,transfer:70}},
L3:{weightedMin:82,floors:{knowledge:85,interpretation:85,production:85,transfer:80}},
L4:{weightedMin:82,floors:{knowledge:90,interpretation:85,production:85,transfer:80,retention:80}},
L5:{weightedMin:84,floors:{knowledge:85,interpretation:85,production:90,transfer:85,retention:80},rubricMin:4},
Expert:{weightedMin:86,floors:{knowledge:85,interpretation:85,production:90,transfer:90,retention:80},rubricMin:4,capstoneMin:85,architectureReview:true}
});
const valid=v=>typeof v==='number'&&Number.isFinite(v)&&v>=0&&v<=100;
export function weightedScore(evidence,weights=DEFAULT_WEIGHTS){for(const key of Object.keys(weights))if(!valid(evidence[key]))throw new Error(`Invalid score for ${key}`);return Math.round(Object.entries(weights).reduce((s,[k,w])=>s+evidence[k]*w,0)*10)/10;}
